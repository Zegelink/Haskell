...
maxInt :: Tree -> Int
maxInt (Leaf i) = i
maxInt (Node a b c) = if (maxInt b) > (maxInt c) then 
    if (a > (maxInt b)) then a
      else maxInt b 
      else if (a > (maxInt c)) then
      a
      else maxInt c
...
isBST :: Tree -> Bool
isBST t = ascendingList(inorder t)
...
--   
inBST :: Int -> Tree -> Bool
inBST n (Leaf a) = if (n == a) then True else False
inBST n (Node a b c) = if (n == a) then True else (inBST n b) || (inBST n c)
